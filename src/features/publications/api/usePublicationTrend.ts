// src/features/publications/api/usePublicationTrend.ts
import { useQuery } from '@tanstack/react-query';
import type { PublicationFilters, PublicationTrend } from '../types';

export function usePublicationTrend(filters: PublicationFilters = {}) {
  return useQuery<PublicationTrend[]>({
    queryKey: ['publications', 'trend', filters],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/publications/trend?${searchParams.toString()}`);
