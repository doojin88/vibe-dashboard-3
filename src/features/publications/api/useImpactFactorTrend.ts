// src/features/publications/api/useImpactFactorTrend.ts
import { useQuery } from '@tanstack/react-query';
import type { PublicationFilters, ImpactFactorTrend } from '../types';

export function useImpactFactorTrend(filters: PublicationFilters = {}) {
  return useQuery<ImpactFactorTrend[]>({
    queryKey: ['publications', 'impact-trend', filters],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/publications/impact-trend?${searchParams.toString()}`);
      if (!response.ok) throw new Error('Failed to fetch impact factor trend');
